...
from tests.kiosk.conftest import ConcretePlaywrightControls
...
async def test_scroll_zero_raises(mock_pw_controls: ConcretePlaywrightControls) -> None:
    with pytest.raises(ValueError, match="greater than 0"):
        await mock_pw_controls.scroll("down", 0)


async def test_scroll_negative_raises(mock_pw_controls: ConcretePlaywrightControls) -> None:
    with pytest.raises(ValueError, match="greater than 0"):
...
async def test_click_negative_x_raises(mock_pw_controls: ConcretePlaywrightControls) -> None:
    with pytest.raises(ValueError, match="non-negative"):
        await mock_pw_controls.click(-1, 0)


async def test_click_negative_y_raises(mock_pw_controls: ConcretePlaywrightControls) -> None:
    with pytest.raises(ValueError, match="non-negative"):
...
async def test_navigate_empty_string_raises(mock_pw_controls: ConcretePlaywrightControls) -> None:
    with pytest.raises(ValueError, match="Invalid URL"):
        await mock_pw_controls.navigate("")


async def test_navigate_no_protocol_raises(mock_pw_controls: ConcretePlaywrightControls) -> None:
    with pytest.raises(ValueError, match="Malformed URL"):
        await mock_pw_controls.navigate("example.com")


async def test_navigate_ftp_raises(mock_pw_controls: ConcretePlaywrightControls) -> None:
    with pytest.raises(ValueError, match="Malformed URL"):
...
async def test_navigate_https_calls_goto(mock_pw_controls: ConcretePlaywrightControls) -> None:
...
async def test_navigate_file_url_calls_goto(mock_pw_controls: ConcretePlaywrightControls, tmp_html) -> None:
    url = f"file://{tmp_html}"
...
async def test_scroll_down_dispatches_wheel(mock_pw_controls: ConcretePlaywrightControls) -> None:
    await mock_pw_controls.scroll("down", 200)
    mock_pw_controls.engine.get_page().mouse.wheel.assert_awaited_once_with(0, 200)


async def test_scroll_left_dispatches_wheel(mock_pw_controls: ConcretePlaywrightControls) -> None:
...
async def test_soft_exception_returns_false(mock_pw_engine: PlaywrightEngine) -> None:
    ...
    class BrokenControls(ConcretePlaywrightControls):
        @control_action
...
async def test_value_error_propagates(mock_pw_controls: ConcretePlaywrightControls) -> None:
...
async def test_runtime_error_propagates(tmp_html) -> None:
    ...
    controls = ConcretePlaywrightControls(engine=engine)
