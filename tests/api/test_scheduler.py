...
async def test_run_schedule_calls_navigate() -> None:
    ...
    request = ScheduleRequest(entries=[
        ScheduleEntry(url="https://a.com", duration_seconds=1, order=1),
        ScheduleEntry(url="https://b.com", duration_seconds=1, order=2),
    ])
...
async def test_ad_break_navigates_to_ad_then_returns() -> None:
    ...
    scheduler = KioskScheduler(registry)
    await scheduler.run_ad_break("uuid-1", AdBreak(url="https://ad.com", duration_seconds=1))
